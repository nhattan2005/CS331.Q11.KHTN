...
class Processor:
    ...
    def generate_merged_masks(self):
        def process_file(filename, save_path, sam_path, merger):
            # Sửa lại cách lấy tên file - loại bỏ extension
            name = os.path.splitext(os.path.basename(filename.path))[0]
            if os.path.isfile(save_path + f"/{name}.png"):
                return
            predict = np.array(Image.open(filename.path))
            sam_folder = os.path.join(sam_path, name)
            
            # Kiểm tra xem sam_folder có tồn tại không
            if not os.path.exists(sam_folder):
                print(f"Warning: SAM folder not found for {name}, skipping...")
                return
    ...
    def evaluate(self):
        ...
        for filename in os.scandir(self.save_path):
            if filename.is_file():
                # Sửa lại cách lấy tên file
                name = os.path.splitext(os.path.basename(filename.path))[0]
                
                # Fix đường dẫn GT - chỉ dùng tên file, không thêm subfolder
                gt_file = os.path.join(self.gt_dir, f'{name}.png')
                pseudo_file = os.path.join(self.pseudo_path, f'{name}.png')
                
                # Kiểm tra file tồn tại
                if not os.path.exists(gt_file):
                    print(f"Warning: GT file not found for {name}, skipping...")
                    continue
                if not os.path.exists(pseudo_file):
                    print(f"Warning: Pseudo file not found for {name}, skipping...")
                    continue
